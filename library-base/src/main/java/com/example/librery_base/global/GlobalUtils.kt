...
class GlobalUtils {
    companion object{
         ...
         * 开眼的版本名
         */
        val eyepetizerVersionName:String="6.3.1"

        /**
         * 开眼的版本号
         */
        val eyepetizerVersionCode:Long=6030012
        ...
        fun init(context: Context){
            ...
            appName=context.resources.getString(context.applicationInfo.labelRes)
            appVersionName=context.packageManager.getPackageInfo(appPackage,0).versionName
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                appVersionCode=context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
            } else {
                appVersionCode=context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
            }
            ...
            if (deviceSerial == null) {
                ...
                if ("google" != appChannel || "samsung" != appChannel) {
                    try {
                        ...
                        Log.d(TAG, "get android_id with error", e)
                    }
                ...
                var uuid = DataStoreUitls.readStringData("uuid", "")
                if (!TextUtils.isEmpty(uuid)) {
                    deviceSerial = uuid
                    return deviceSerial.toString()
                }
                uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase(Locale.getDefault())
                CoroutineScope(Dispatchers.IO).launch { DataStoreUitls.saveStringDataAsync("uuid", uuid) }
                deviceSerial = uuid
                return deviceSerial.toString()
            } else {
                return deviceSerial.toString()
        ...
        fun getResourceId(name: String, type: String): Int {
            return context?.resources?.getIdentifier(name, type, appPackage)!!
        }
        ...
        fun getApplicationMetaData(key: String): String? {
            ...
            try {
                applicationInfo = context?.packageManager?.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
